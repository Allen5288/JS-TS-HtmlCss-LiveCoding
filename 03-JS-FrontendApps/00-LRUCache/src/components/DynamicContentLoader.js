...
/**
 * DynamicContentLoader demonstrates a practical use case for LRU Cache
...
function DynamicContentLoader() {
  ...
  // Use ref to store cache keys for comparing before and after states
   ...
   */
  const handleTabClick = useCallback(async (id) => {
    setActiveTab(id);
    ...
    if (cachedContent !== -1) {
      ...
      // Store current cache keys before update
      if (entries) {
        cacheKeysRef.current = entries.map(e => parseInt(e.key));
      }
      ...
      // Check for evicted items - only when cache was at capacity
      if (cacheKeysRef.current.length === 3) {
        // We need to wait for the next render cycle when entries has been updated
        setTimeout(() => {
          
          const currentCacheKeys = entries.map(e => parseInt(e.key));
          const evicted = cacheKeysRef.current.find(
          );
          
          if (evicted !== undefined) {
            setEvictedItems(prev => [
              { id: evicted, timestamp: new Date().toLocaleTimeString() },
  ...
  }, [get, put, fetchTabContent]); // Removed entries dependency

  return (
    <div className="dynamic-loader">
      <h2>Tab Content with LRU Cache</h2>
      <p className="description">
        This demonstrates a real-world use case of LRU Cache. The last 3 accessed tabs are cached.
        When you click on more than 3 different tabs, older ones get evicted.
      </p>
        ...
        activeTab && activeContent && (
          <div className="tab-content">
            ...
            <div className="content-box">
              ...
              <p className="source">
                {contentSource === 'cache' ? "Retrieved from cache" : "Freshly loaded from server"}
              </p>
            </div>
          </div>
        )
      ...
      <div className="cache-stats">
        ...
        <p>Cache Hits: {cacheHits} | Cache Misses: {cacheMisses}</p>
      </div>
          ...
          {entries && entries.length > 0 ? (
            <ul>
              {entries.map((entry, index) => (
                <li 
                  key={index} 
                  className={`${activeTab === parseInt(entry.key) ? 'active-item' : ''} ${entry.isMostRecent ? 'most-recent' : ''} ${entry.isLeastRecent ? 'least-recent' : ''}`}
                >
