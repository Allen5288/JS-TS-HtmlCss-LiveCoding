...
class ClimbingStairs {
    ...
    climbStairsMatrix(n) {
        ...
        // [F(n), F(n-1)] = [F(2), F(1)] * baseMatrix^(n-1) = [2, 1] * resultMatrix
        return 2 * resultMatrix[0][0] + 1 * resultMatrix[1][0];
    }
    /**
     ...
     */
    static runTests() {
        TestHelper_1.TestHelper.printHeader('LeetCode 70. Climbing Stairs');
        ...
        const n = 40;
        for (const approach of approaches) {
            TestHelper_1.TestHelper.measureTime(() => approach.method(n), approach.name);
        }
        ...
        for (let i = 0; i < efficientApproaches.length; i++) {
            ...
            TestHelper_1.TestHelper.runTest(`${efficientNames[i]} (n = ${largeN})`, expected, result);
        }
        console.log('\n📝 Algorithm Analysis:');
