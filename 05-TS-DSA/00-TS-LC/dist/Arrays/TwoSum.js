...
class TwoSum {
     ...
     */
    static runTests() {
        TestHelper_1.TestHelper.printHeader('LeetCode 1. Two Sum');
        ...
        for (const testCase of testCases) {
            ...
            TestHelper_1.TestHelper.runTest(`${testCase.name} (Brute Force)`, testCase.expected, result);
        }
        ...
        for (const testCase of testCases) {
            ...
            TestHelper_1.TestHelper.runTest(`${testCase.name} (Two Pointers)`, testCase.expected, result);
        }
        // Performance comparison
        console.log('\n⚡ Performance Comparison:');
        const largeArray = Array.from({ length: 1000 }, (_, i) => i);
        const largeTarget = 1998; // Last two elements
        TestHelper_1.TestHelper.measureTime(() => solution.twoSumHashMap(largeArray, largeTarget), 'Hash Map (1000 elements)');
        TestHelper_1.TestHelper.measureTime(() => solution.twoSumTwoPointers(largeArray, largeTarget), 'Two Pointers (1000 elements)');
